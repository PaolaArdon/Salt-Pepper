...
// TODO give recognition message as an argument
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const RecognitionMessage &recog)
{
    if(mSensor!=RGBD)
    {
    ...
    return mpTracker->GrabImageRGBD(im,depthmap,timestamp, recog);
}

cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp, const RecognitionMessage &recog)
{
    cv::Mat Tcw;
    if(mSensor!=MONOCULAR)
    {
    ...
    return (mpTracker->GrabImageMonocular(im,timestamp, recog));

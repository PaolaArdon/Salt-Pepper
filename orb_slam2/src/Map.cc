...
void Map::AddRecognitionPoint(RecognitionMessage* rec)
{
    unique_lock<mutex> lock(mMutexMap);
    mspRecognitionPoints.insert(rec);
...
void Map::EraseRecognitionPoint(RecognitionMessage *pRP)
{
    unique_lock<mutex> lock(mMutexMap);
    mspRecognitionPoints.erase(pRP);

    // TODO: This only erases the pointer.
    // Delete the Recognition Point
}
...
vector<RecognitionMessage*> Map::GetAllRecognitionPoints()
{
    unique_lock<mutex> lock(mMutexMap);
    return vector<RecognitionMessage*>(mspRecognitionPoints.begin(),mspRecognitionPoints.end());
}

...
// TODO add recognition message as an argument
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, const RecognitionMessage &recog)
{

    rec = new RecognitionMessage(recog);
    if(rec->flag)
    {
        pangolin::OpenGlMatrix Twc;
        Twc.SetIdentity();
        mpMapDrawer->GetCurrentOpenGLCameraMatrix(Twc);
        // std::cout << "here" << *Twc.m << std::endl;
        std::vector<float> v(3);
        v[0] = Twc(0,3)+Twc(0,2)*rec->depth;
        v[1] = Twc(1,3)+Twc(1,2)*rec->depth;
        v[2] = (Twc(2,3)+Twc(2,2)*rec->depth)/6.0;
        rec->SetWorldPos(v);
        mpMap->AddRecognitionPoint(rec);
        // std::vector<RecognitionMessage*> a = mpMap->GetAllRecognitionPoints();
        // std::cout << "Tracking: object should be added. size: " << a.size() << std::endl;
    }
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, const RecognitionMessage &recog)
{
    // add recognized object info to map
    rec = new RecognitionMessage(recog);
    if(rec->flag)
    {
        pangolin::OpenGlMatrix Twc;
